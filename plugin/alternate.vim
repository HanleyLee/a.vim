...
let s:alternateExtensionsDict = {}
let s:defaultAlternateExtensionsDic = {
            \'h':"c,cpp,cxx,cc,CC",
            \'H':"C,CPP,CXX,CC",
            \'hpp':"cpp,c",
            \'HPP':"CPP,C",
...
function! s:ArrangeExtensionsDic()
    let s:alternateExtensionsDict = s:defaultAlternateExtensionsDic

    if (exists('g:extraAlternativeExtensionsDic'))
        for [key, value] in items(g:extraAlternativeExtensionsDic)
            let alternatives = ''
            if has_key(s:alternateExtensionsDict, key)
                let alternatives = s:alternateExtensionsDict[key] . ',' .value
            else
                let alternatives = value
            endif
            let s:alternateExtensionsDict[key] = alternatives
        endfor
    endif

    if (exists('g:overrideAlternativeExtensionsDic'))
        for [key, value] in items(g:overrideAlternativeExtensionsDic)
            let s:alternateExtensionsDict[key] = value
        endfor
    endif

    for key in keys(s:alternateExtensionsDict)
        let dotsNumber = strlen(substitute(key, "[^.]", "", "g"))
...
function! s:FindFileInSearchPath(fileName, pathList, relPathBase)
    let filepath = ""
    let m = 1
    let pathListLen = strlen(a:pathList)
    if (pathListLen > 0)
        while (1)
            let pathSpec = s:GetNthItemFromList(a:pathList, m)
            if (pathSpec != "")
                let path = s:ExpandAlternatePath(pathSpec, a:relPathBase)
                let fullname = path . "/" . a:fileName
                let foundMatch = s:BufferOrFileExists(fullname)
                if (foundMatch)
...
" Returns  : An expanded filename if found, the empty string otherwise
" Author   : Michael Sharpe (feline@irendi.com)
" History  : Based on s:FindFileInSearchPath() but with extensions
...
" Author   : Michael Sharpe <feline@irendi.com>
function! s:EnumerateFilesByExtension(path, baseName, extension)
    let enumeration = ""
    let extSpec = get(s:alternateExtensionsDict, a:extension, '')
    if (extSpec != "")
        let n = 1
        let done = 0
        while (!done)
            let ext = s:GetNthItemFromList(extSpec, n)
            if (ext != "")
                if (a:path != "")
...
" Author   : Michael Sharpe <feline@irendi.com>
function! s:EnumerateFilesByExtensionInPath(baseName, extension, pathList, relPathBase)
    let enumeration = ""
    let filepath = ""
    let m = 1
    let pathListLen = strlen(a:pathList)
    if (pathListLen > 0)
        while (1)
            ...
            if (pathSpec != "")
                let path = s:ExpandAlternatePath(pathSpec, a:relPathBase)
                let pe = s:EnumerateFilesByExtension(path, a:baseName, a:extension)
                if (enumeration == "")
                    let enumeration = pe
                else
...
"            message.
function! s:DetermineExtension(path)
    let mods = ":t"
    let i = 0
...
function! AlternateFile(splitWindow, ...)
    let extension   = s:DetermineExtension(expand("%:p"))
    let baseName    = substitute(expand("%:t"), "\." . extension . '$', "", "")
    let currentPath = expand("%:p:h")

    if (a:0 != 0)
        let newFullname = currentPath . "/" .  baseName . "." . a:1
        call s:FindOrCreateBuffer(newFullname, a:splitWindow, 0)
    else
        let allfiles = ""
        if (extension != "")
            let allfiles1 = s:EnumerateFilesByExtension(currentPath, baseName, extension)
            let allfiles2 = s:EnumerateFilesByExtensionInPath(baseName, extension, g:alternateSearchPath, currentPath)
        ...
        if (allfiles != "")
            ...
            let onefile = s:GetNthItemFromList(allfiles, n)
            let bestFile = onefile
            while (onefile != "" && score < 2)
                let score = s:BufferOrFileExists(onefile)
                if (score > bestScore)
                    let bestScore = score
                    let bestFile = onefile
                endif
            ...
            if (bestScore == 0 && g:alternateNoDefaultAlternate == 1)
                ...
                call s:FindOrCreateBuffer(bestFile, a:splitWindow, 1)
...
function! AlternateOpenFileUnderCursor(splitWindow,...)
    ...
    let fileName = s:FindFileInSearchPathEx(cursorFile, g:alternateSearchPath, currentPath, openCount)
    if (fileName != "")
        call s:FindOrCreateBuffer(fileName, a:splitWindow, 1)
...
function! AlternateOpenNextFile(bang)
    ...
    if (cursorFile != ""  && currentPath != ""  && openCount != 0)
        let fileName = s:FindFileInSearchPathEx(cursorFile, g:alternateSearchPath, currentPath, openCount)
        if (fileName != "")
            call s:FindOrCreateBuffer(fileName, "n".a:bang, 0)
            let b:openCount = openCount
            let b:cursorFile = cursorFile
            let b:currentPath = currentPath
        else
            let fileName = s:FindFileInSearchPathEx(cursorFile, g:alternateSearchPath, currentPath, 1)
            if (fileName != "")
                call s:FindOrCreateBuffer(fileName, "n".a:bang, 0)
                let b:openCount = 1
                let b:cursorFile = cursorFile
...
"function! s:PrintList(theList)
"   let n = 1
"   let oneFile = s:GetNthItemFromList(a:theList, n)
"   while (oneFile != "")
"      let n = n + 1
"      let oneFile = s:GetNthItemFromList(a:theList, n)
"   endwhile
...
function! NextAlternate(bang)
    if (exists('b:AlternateAllFiles'))
        ...
        let n = 1
        let onefile = s:GetNthItemFromList(b:AlternateAllFiles, n)
        while (onefile != "" && !s:EqualFilePaths(fnamemodify(onefile,":p"), fnamemodify(currentFile,":p")))
            let n = n + 1
            let onefile = s:GetNthItemFromList(b:AlternateAllFiles, n)
        endwhile
        ...
        if (onefile != "")
            ...
            let nextAlternate = ""
            while (n != stop)
                let nextAlternate = s:GetNthItemFromList(b:AlternateAllFiles, n)
                if (nextAlternate == "")
                    let n = 1
                    continue
                endif
                let n = n + 1
                if (s:EqualFilePaths(fnamemodify(nextAlternate, ":p"), fnamemodify(currentFile, ":p")))
                    continue
                endif
            ...
            if (foundAlternate == 1)
                ...
                call s:FindOrCreateBuffer(nextAlternate, "n".a:bang, 0)
...
function! s:BufferOrFileExists(fileName)
    let result = 0
